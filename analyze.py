...
def main(args):
    ...
    # plot fitness
    # plot species
    plot_mean_and_bootstrapped_ci_over_time([np.array(c["species_results"]) for c in data], [np.array(c["species_results"]) for c in data], [c["name"] for c in data], "Generation", "N Species", plot_bootstrap=bootst)
    plot_mean_and_bootstrapped_ci_over_time([np.array(c["threshold_results"]) for c in data], [np.array(c["threshold_results"]) for c in data], [np.array(c["name"]) for c in data], "Generation", "Species Threshold", plot_bootstrap=bootst)

    plot_mean_and_bootstrapped_ci_over_time([np.array(c["nodes_results"]) for c in data], [np.array(c["nodes_results"]) for c in data], [np.array(c["name"]) for c in data], "Generation", "Number of Nodes", plot_bootstrap=bootst)
    plot_mean_and_bootstrapped_ci_over_time([np.array(c["connections_results"]) for c in data], [np.array(c["connections_results"]) for c in data], [c["name"] for c in data], "Generation", "Number of Connections", plot_bootstrap=bootst)
